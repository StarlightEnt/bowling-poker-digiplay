...
export default function HandDisplay({ hand, gameNumber, isSubmitted, isForfeited, cardSize = 'md' }) {
  if (!hand) return null;
  ...
  return (
    <div style={{
      background: '#16213e',
      border: `1px solid ${isRoyalFlush ? '#ffd700' : '#2a2a5a'}`,
      borderRadius: 8,
      padding: 16,
      display: 'flex',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {totalCards > 0 && (
          <div style={{ color: '#555577', fontSize: 11 }}>Total: {totalCards}</div>
        )}
      </div>
