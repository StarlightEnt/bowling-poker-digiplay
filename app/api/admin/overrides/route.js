...
export async function POST(request) {
  ...
  // adjust_draw_count
  else if (action === 'adjust_draw_count') {
    const newCount = parseInt(value);
    if (isNaN(newCount) || newCount < 0) return Response.json({ error: 'Invalid count' }, { status: 400 });
    await sql`UPDATE player_game_state SET cards_drawn = ${newCount}
      WHERE player_id = ${playerId} AND game_id = ${gameId}`;
    details = { newCount };
    result = { success: true };
  }
